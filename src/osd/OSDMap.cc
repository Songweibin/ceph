  ...
  OSDMap::Incremental *pending_inc)
{
  ...
  auto max_stddev = cct->_conf->get_val<double>("osd_calc_pg_upmaps_max_stddev");
  ...
  while (max--) {
    ...
    for (auto it = ordered_pgs_by_primary.begin();
      ...
      ldout(cct, 20) << " primary osd." << primary
                     ...
                     << "\tdeviation " << deviation
                     << dendl;
      // note that we double the maximum allowed deviation ratio here
      // since there are usually fewer than half of the total pgs
      // that are primaries
      if (it == ordered_pgs_by_primary.begin() &&
          deviation / target < max_deviation_ratio * 2) {
        ldout(cct, 10) << " deviation_ratio " << deviation / target
                       << " < max_deviation_ratio " << max_deviation_ratio * 2
                       << dendl;
        break;
      }
      stddev += deviation * deviation;
